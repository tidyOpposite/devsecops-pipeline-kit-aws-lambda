...
| Immutable Lambda image | `lambda_image_uri`, `devsecops image validate` | Root and Lambda module validation reject mutable `latest`/`bootstrap`; Lambda module precondition blocks apply without an image. | Deploy job requires `LAMBDA_IMAGE_URI` and rejects mutable tags. | Lambda is updated to the configured immutable image; rollback restores previous image. | Snyk scans the configured image when enabled. |
...
| Deployment rollback | `devsecops deploy status`, `deploy logs`, `deploy rollback` | Re-applies Terraform with the selected previous immutable image after rollback. | Deploy job captures/restores automatically on failure; operator rollback uses the same protected Environment, OIDC role, and serialized workflow lane. | The workflow updates Lambda; the local CLI never performs a direct cloud write. | Automatic rollback is triggered by failed enabled gates; the same gates validate an operator rollback. |
